...
bool Grid::intersect(const Ray &r, Hit &h, float tmin){
    ...
    if(mi.getIntersect()==0)
        ...
        while(mi.getNear()<mi.getFar()&&i<nx&&i>=0&&j<ny&&j>=0&&k<nz&&k>=0){
            if(stats==1) raystats.IncrementNumGridCellsTraversed();
...
bool Grid::fastRayCast(const Ray &r, Hit &h, float tmin){
    MarchingInfo mi;
    initializeRayMarch(mi, r, tmin);

    Hit hitinf = h;
    bool flaginf = 0;
    for (int ind = 0; ind < infnum;ind++){
        Object3D *o = infinite.getObject(ind);
        if(o->intersect(r, hitinf, tmin))
            flaginf = 1;
    }

    if(mi.getIntersect()==0&&flaginf==0)
        return false;
    else if(mi.getIntersect()==1){
        Vec3f max = bb->getMax(), min = bb->getMin();
        float gridx = (max - min).x() / nx, gridy = (max - min).y() / ny, gridz = (max - min).z() / nz;
        Vec3f x(gridx, 0, 0), y(0, gridy, 0), z(0, 0, gridz);   
        int i = mi.geti(), j = mi.getj(), k = mi.getk();  
        Vec3f minvox(i * gridx, j * gridy, k * gridz);
        Vec3f maxvox = minvox + Vec3f(gridx, gridy, gridz);
        ...
        while (mi.getNear() < mi.getFar() && i < nx && i >= 0 && j < ny && j >= 0 && k < nz && k >= 0)
        {
            ...
            if (this->getGrid(i, j, k) >= 1)
            {
                bool flag = false;
                Hit hmin = h;
                for (int ind = 0; ind < this->getGrid(i, j, k); ind++)
                {
                    Object3D *o = this->getObj(i, j, k, ind);
                    Hit htemp = h;
                    Matrix mat = o->getMat();
                    Transform trans(mat, o);
                    if (trans.intersect(r, htemp, tmin))
                    {
                        Vec3f point = r.pointAtParameter(htemp.getT()) - min; // 相对位置
                        if (Vec3f::minVec(point, minvox) == minvox && Vec3f::maxVec(point, maxvox) == maxvox)
                        {
                            // 如果交点位置在该cell内
                            flag = true;
                            if (htemp.getT() < hmin.getT())
                            {
                                hmin = htemp;
                            }
                        }
            ...
            mi.nextCell();
            i = mi.geti(), j = mi.getj(), k = mi.getk();
            minvox = Vec3f(i * gridx, j * gridy, k * gridz);
            maxvox = minvox + Vec3f(gridx, gridy, gridz);
            // 更新信息
        }
        if(flaginf==0) return false; // 循环结束仍未找到
        else{
            h = hitinf;
            return true;
        }
