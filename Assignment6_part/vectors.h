...
public:
  ...
  friend Vec3f operator*(const Vec3f &v1,const Vec3f &v2){
    Vec3f v3(v1.data[0] * v2.data[0], v1.data[1] * v2.data[1], v1.data[2] * v2.data[2]);
    return v3;
  }  
  ...
  float Dot3(const Vec3f &V) const {
    return data[0] * V.data[0] +
      data[1] * V.data[1] +
      data[2] * V.data[2] ; }
  static Vec3f minVec(const Vec3f &v1,const Vec3f &v2){
    float minx = min(v1.x(), v2.x()), miny = min(v1.y(), v2.y()), minz = min(v1.z(), v2.z());
    return Vec3f(minx, miny, minz);
  }
  static Vec3f maxVec(const Vec3f &v1,const Vec3f &v2){
    float maxx = max(v1.x(), v2.x()), maxy = max(v1.y(), v2.y()), maxz = max(v1.z(), v2.z());
